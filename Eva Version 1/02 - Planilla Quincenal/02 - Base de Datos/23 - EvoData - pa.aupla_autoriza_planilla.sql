IF EXISTS ( SELECT  *
            FROM    sys.objects
            WHERE   object_id = OBJECT_ID(N'pa.aupla_autoriza_planilla')
                    AND type IN ( N'P', N'PC' ) )
/****** Object:  StoredProcedure [pa].[aupla_autoriza_planilla]    Script Date: 16-01-2017 2:23:40 PM ******/
DROP PROCEDURE [pa].[aupla_autoriza_planilla]
GO

CREATE PROCEDURE [pa].[aupla_autoriza_planilla]
   @codppl INT,
...
-- Autoriza las asistencias que se encuentran en estado Pendiente
update rel.asi_asistencias
set asi_estado = 'Autorizado'
where asi_codppl = @codppl
and asi_estado = 'Pendiente'
...
update sal.dcc_descuentos_ciclicos
   set dcc_total_cobrado = isnull(total_cobrado, 0.00) + isnull(total_extras, 0.00),
       dcc_total_no_cobrado = isnull(total_no_cobrado, 0.00),
       dcc_activo = (case when dcc_usa_porcentaje = 0
					 then (case when dcc_monto_indefinido = 0 and 
                              ((isnull(total_cobrado, 0.00) + isnull(total_extras, 0.00)) >= dcc_monto or isnull(maxima_cuota, 0) >= dcc_numero_cuotas)
                           then 0 
                           else 1
						   end)
					 else (case when (isnull(total_cobrado, 0.00) + isnull(total_extras, 0.00)) >= isnull(gen.get_pb_field_data_float(dcc_property_bag_data, 'dcc_saldo_impar'), 0.00) and isnull(gen.get_pb_field_data_float(dcc_property_bag_data, 'dcc_saldo_impar'), 0.00) > 0
						   then 0
						   else 1
						   end)
					 end),
	   dcc_property_bag_data = (case when dcc_usa_porcentaje = 1 and isnull(gen.get_pb_field_data_float(dcc_property_bag_data, 'dcc_saldo_impar'), 0.00) > 0.00
								then gen.set_pb_field_data(dcc_property_bag_data, 'DescuentosCiclicos', 'dcc_saldo_impar', convert(numeric(12,2), isnull(gen.get_pb_field_data_float(dcc_property_bag_data, 'dcc_monto_impar'), 0.00) - isnull(total_cobrado, 0.00) - isnull(total_extras, 0.00)))
								else dcc_property_bag_data
								end)
...
-- FONDO DE VACACIONES
--if @codtplVisual = @codtpl_quincenal
--	exec pa.GenPla_Genera_Fondo_Vacacion @codppl, @userName
...
IF @tpl_aplicacion = 'Normal' AND 1 = 2
BEGIN
