...
select @codcia as dco_codcia,
	   ...
	   into #Provision_SS_D
from #Provision_SS_C
join sal.trs_tipos_reserva on trs_codcia=@codcia and trs_abreviatura='PROV_SSPatronal' 
left outer join sal.ctr_cuentas_tipo_reserva on ctr_codtrs = trs_codigo
where dco_codppl = @codppl
...
select @codcia as dco_codcia,
	 ...
	 Round(sum(dco_debitos),2) dco_creditos,
	   ...
	   into #Provision_SE_D
from #Provision_SE_C
join sal.trs_tipos_reserva on trs_codcia=@codcia AND trs_abreviatura='PROV_SEPatronal' 
left outer join sal.ctr_cuentas_tipo_reserva on ctr_codtrs = trs_codigo
where dco_codppl = @codppl
group by dco_codppl, ctr_cuenta_aux, trs_abreviatura
