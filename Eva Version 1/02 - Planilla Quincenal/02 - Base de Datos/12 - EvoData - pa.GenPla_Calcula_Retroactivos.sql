IF EXISTS ( SELECT  *
            FROM    sys.objects
            WHERE   object_id = OBJECT_ID(N'pa.GenPla_Calcula_Retroactivos')
                    AND type IN ( N'P', N'PC' ) )

/****** Object:  StoredProcedure [pa].[GenPla_Calcula_Retroactivos]    Script Date: 16-01-2017 11:16:43 AM ******/
DROP PROCEDURE [pa].[GenPla_Calcula_Retroactivos]
GO
...
CREATE PROCEDURE [pa].[GenPla_Calcula_Retroactivos](
...
SELECT @ppl_fecha_ini = PPL_FECHA_INI, @ppl_fecha_fin = PPL_FECHA_FIN, @codcia = tpl_codcia, @codpai = cia_codpai
FROM sal.ppl_periodos_planilla (nolock)
JOIN sal.tpl_tipo_planilla (nolock) on tpl_codigo = ppl_codtpl
JOIN eor.cia_companias (nolock) on cia_codigo = tpl_codcia
...
JOIN exp.fn_get_datos_rubro_salarial(null, 'S', @ppl_fecha_fin, @codpai) 
...
BEGIN
	...
	SET @retroactivo = COALESCE(@pago_salario_nuevo - @pago_salario_previo, 0)
