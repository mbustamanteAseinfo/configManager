...
IF EXISTS ( SELECT  *
            FROM    sys.objects
            WHERE   object_id = OBJECT_ID(N'pa.aju_ajustes')
                    AND type IN ( N'S', N'U' ) )
					begin
ALTER TABLE [pa].[aju_ajustes] DROP CONSTRAINT [fk_salppl_pa_aju_];
alter table [pa].[aju_ajustes] DROP CONSTRAINT [pk_pa_aju_];
ALTER TABLE [pa].[aju_ajustes] DROP CONSTRAINT [fk_expemp_pa_aju_];

/****** Object:  Table [pa].[dco_datos_contables]    Script Date: 16-01-2017 2:20:24 PM ******/
DROP TABLE [pa].[aju_ajustes];
end
GO
