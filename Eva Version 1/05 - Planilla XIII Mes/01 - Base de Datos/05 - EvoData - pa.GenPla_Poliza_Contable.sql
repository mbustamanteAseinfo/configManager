...
join sal.trs_tipos_reserva on trs_codigo = res_codtrs and trs_codcia=@codcia AND trs_abreviatura='PROV_SSPatronal_PA' 
...
select @codcia as dco_codcia,
	   ...
	   into #Provision_SS_D
from #Provision_SS_C
join sal.trs_tipos_reserva on trs_codcia=@codcia and trs_abreviatura='PROV_SSPatronal_PA' 
left outer join sal.ctr_cuentas_tipo_reserva on ctr_codtrs = trs_codigo
where dco_codppl = @codppl
...
into #Provision_SE_C
from sal.res_reservas
join sal.trs_tipos_reserva on trs_codigo = res_codtrs and trs_codcia=@codcia AND trs_abreviatura='PROV_SEPatronal_PA' 
left outer join sal.ctr_cuentas_tipo_reserva on ctr_codtrs = trs_codigo
join exp.emp_empleos on res_codemp = emp_codigo
join eor.plz_plazas on emp_codplz = plz_codigo
left outer join eor.cpp_centros_costo_plaza on plz_codigo = cpp_codplz
...
select @codcia as dco_codcia,
	 ...
	 Round(sum(dco_debitos),2) dco_creditos,
	   ...
	   into #Provision_SE_D
from #Provision_SE_C
join sal.trs_tipos_reserva on trs_codcia=@codcia AND trs_abreviatura='PROV_SEPatronal_PA' 
left outer join sal.ctr_cuentas_tipo_reserva on ctr_codtrs = trs_codigo
where dco_codppl = @codppl
group by dco_codppl, ctr_cuenta_aux, trs_abreviatura
...
into #Provision_A_C
from sal.res_reservas
join sal.trs_tipos_reserva on trs_codigo = res_codtrs and trs_codcia=@codcia and trs_abreviatura='PROV_XIII_PA'
left outer join sal.ctr_cuentas_tipo_reserva on ctr_codtrs = trs_codigo
join exp.emp_empleos on res_codemp = emp_codigo
join eor.plz_plazas on emp_codplz = plz_codigo
left outer join eor.cpp_centros_costo_plaza on plz_codigo = cpp_codplz
...
select @codcia as dco_codcia,
	   ...
	   into #Provision_A_D
from #Provision_A_C
join sal.trs_tipos_reserva on trs_codcia=@codcia and trs_abreviatura='PROV_XIII_PA'
left outer join sal.ctr_cuentas_tipo_reserva on ctr_codtrs = trs_codigo
where dco_codppl = @codppl
group by dco_codppl, ctr_cuenta_aux, trs_abreviatura
...
into #Provision_B_C
from sal.res_reservas
join sal.trs_tipos_reserva on trs_codigo = res_codtrs and trs_codcia=@codcia and trs_abreviatura='PROV_Prima_Anti_PA'
left outer join sal.ctr_cuentas_tipo_reserva on ctr_codtrs = trs_codigo
join exp.emp_empleos on res_codemp = emp_codigo
join eor.plz_plazas on emp_codplz = plz_codigo
left outer join eor.cpp_centros_costo_plaza on plz_codigo = cpp_codplz
...
select @codcia as dco_codcia,
	   ...
	   into #Provision_B_D
from #Provision_B_C
join sal.trs_tipos_reserva on trs_codcia=@codcia and trs_abreviatura='PROV_Prima_Anti_PA'
left outer join sal.ctr_cuentas_tipo_reserva on ctr_codtrs = trs_codigo
where dco_codppl = @codppl
group by dco_codppl, ctr_cuenta_aux, trs_abreviatura
...
into #Provision_V_C
from sal.res_reservas
join sal.trs_tipos_reserva on trs_codigo = res_codtrs and trs_codcia=@codcia and trs_abreviatura='PROV_VAC_PA'
left outer join sal.ctr_cuentas_tipo_reserva on ctr_codtrs = trs_codigo
join exp.emp_empleos on res_codemp = emp_codigo
join eor.plz_plazas on emp_codplz = plz_codigo
left outer join eor.cpp_centros_costo_plaza on plz_codigo = cpp_codplz
...
select @codcia as dco_codcia,
	   ...
	   into #Provision_V_D
from #Provision_V_C
join sal.trs_tipos_reserva on trs_codcia=@codcia and trs_abreviatura='PROV_VAC_PA'
left outer join sal.ctr_cuentas_tipo_reserva on ctr_codtrs = trs_codigo
where dco_codppl = @codppl
group by dco_codppl, ctr_cuenta_aux, trs_abreviatura
...
into #Provision_I_C
from sal.res_reservas
join sal.trs_tipos_reserva on trs_codigo = res_codtrs and trs_codcia=@codcia and trs_abreviatura='PROV_INDEM_PA'
left outer join sal.ctr_cuentas_tipo_reserva on ctr_codtrs = trs_codigo
join exp.emp_empleos on res_codemp = emp_codigo
join eor.plz_plazas on emp_codplz = plz_codigo
left outer join eor.cpp_centros_costo_plaza on plz_codigo = cpp_codplz
...
select @codcia as dco_codcia,
	   ...
	   into #Provision_I_D
from #Provision_I_C
join sal.trs_tipos_reserva on trs_codcia=@codcia and trs_abreviatura='PROV_INDEM_PA'
left outer join sal.ctr_cuentas_tipo_reserva on ctr_codtrs = trs_codigo
where dco_codppl = @codppl
group by dco_codppl, ctr_cuenta_aux, trs_abreviatura
...
into #Provision_R_C
from sal.res_reservas
join sal.trs_tipos_reserva on trs_codigo = res_codtrs and trs_codcia=@codcia and trs_abreviatura='PROV_Riesgo_PA'
left outer join sal.ctr_cuentas_tipo_reserva on ctr_codtrs = trs_codigo
join exp.emp_empleos on res_codemp = emp_codigo
join eor.plz_plazas on emp_codplz = plz_codigo
left outer join eor.cpp_centros_costo_plaza on plz_codigo = cpp_codplz
...
SELECT @codcia AS dco_codcia,
	   ...
	   INTO #Provision_R_D
FROM #Provision_I_C
JOIN sal.trs_tipos_reserva ON trs_codcia=@codcia AND trs_abreviatura='PROV_Riesgo_PA'
LEFT OUTER JOIN sal.ctr_cuentas_tipo_reserva ON ctr_codtrs = trs_codigo
WHERE dco_codppl = @codppl
GROUP BY dco_codppl, ctr_cuenta_aux, trs_abreviatura
